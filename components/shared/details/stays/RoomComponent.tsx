  ...
  const handleBookNow = (event: any) => {
    ...
    requestStayBooking(props.room.id, props.checkin, props.checkout)
      .then((data) => {
        if (data.booking_id) {
          router.push(
            `/find-stays/stay-booking/${props.stayId}?booking_id=${data.booking_id}&checkin=${props.checkin}&checkout=${props.checkout}&expiration=${data.lock_expiration}`
          );
        } else {
          toast({
