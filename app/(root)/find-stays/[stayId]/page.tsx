  ...
  const onPostReview = (description: string, rating: number) => {
    ...
    return (
      <main className="flex w-full flex-col gap-4 py-4">
        ...
        <ReviewsSection paginationModel={paginationModel} onPageChange={() => onPageChange(paginationModel.page)} reviewCount={stayData?.review_count || 0} averageRating={stayData?.rating || 0} type="stay" reviews={reviews} id={Number(stayData?.id)} onGiveReview={() =>setIsAddReviewModalOpen(true)}/>
